...
- **Outcome:** DONE — 1 build attempt, QA PASS (light track, no review, branch dev-team/model-migration-run, commit 3cac7b5)
- **What happened:** Sonnet engineer authored docs/architecture/hosted-web-model.md (Context/Decision/Consequences ADR) from PLAN.md locked-decisions table: pricing+seats+BYOK, hosting stack + operating cost + 4-row risk/mitigation table, per-client-repo decision with explicit Part II reversal note, package/template propagation. QA committed a node check (docs/architecture/__tests__/hosted-web-model.check.mjs, 18/18).
