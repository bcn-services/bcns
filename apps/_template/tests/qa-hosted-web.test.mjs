/**
 * QA value-add tests for the app template.
 * Complements the ai-optin/shared-mode/rls suites. Run with:
...
test("env: getConfig returns safe defaults when no keys are set (no throw)", () => {
  ...
  try {
    ...
    assert.equal(cfg.hasServiceRoleKey, false, "no service key present");
