...
4. **What happens if I want to cancel?** — Cancel any time; access continues 30 days after you cancel, then the service ends; you can request an export of your data any time before it is deleted, about 60 days after you cancel.
...
**`legal.privacy`/`legal.terms`** would use a separate `[TODO: ...]` marker, not `[INPUT: ...]`, for a fact the repo doesn't know yet, rather than a copy slot Nate always fills — both render visibly, on purpose. As of 2026-09-22 there are none; all five resolved: the registered agent line was dropped rather than filled; the contact-form vendor is confirmed Web3Forms in both spots (`legal.privacy` → "What we collect" and "Who we share it with"); the Google Drive/Meet-over-MCP lawyer-review flag was dropped; the DPA is documented as "available on request" with no link (`legal.terms` → "Data processing"); and the cancellation-timing question (`legal.terms` → "Term, cancellation and suspension") was decided: access continues 30 days after cancel, then the account ends, and deletion is permitted 30 days after that, about 60 days after cancel total. See `docs/architecture/legal-pages-research.md` for the underlying facts.
