...
- **Next:** The **past-work case study system is built** — all 8 items above the STOP marker are done on branch `worktree-past-work-case-studies` (draft PR #14). `/work` cards link through to `/work/[slug]` detail pages that render real screenshots from locally-hosted client apps on invented demo data. The run stopped at the `⚠️ AUTONOMOUS RUN — STOP HERE` marker. Item 9 (real client copy + publish) is Needs-Nate and cannot start until the permission gate clears.
- **Blockers (Needs-Nate):** Written client permission from L2 Detailz and DeLuca's before either is named publicly; real case study copy (every narrative field, and now every screenshot `caption`, still ships as `[INPUT: …]` and renders as visible bracketed placeholder text on the live detail pages); L2 Detailz launch + DNS cutover before its live link can be published. First live deploy of a client app pending. DeLuca's packaging/handoff tracked in `bcns-client-delucas`.
- **Security follow-up (Needs-Nate, unrelated to any plan item):** `~/bcns-client-l2detailz/.claude/worktrees/demo-seed/.env.local` carries **live production credentials** — real project URL, anon key, **service-role key**, and a production pooler `DATABASE_URL`. `lib/db/index.ts` selects the production Supabase adapter on the mere presence of `NEXT_PUBLIC_SUPABASE_URL`, checked *before* `DATABASE_URL`, so a plain `pnpm dev` in that worktree reads real customer records. Decide whether a demo/screenshot worktree should hold production service-role credentials at all. Not edited by this run.
- **Last updated:** 2026-07-28 (dev-team-auto: items 7–8 done, case study system complete to the STOP marker)
...
| 4 — Link Past Work cards to detail pages | ✅ done [light] — each card wrapped in `<Link href="/work/<slug>">` (grid cell is now `flex h-full flex-col`, Link is `block flex-1`); optional external `link` anchor moved OUT to a sibling — kept, not deleted, since item 9 populates L2's URL; `hover-lift` added to the Card root for click affordance; no new copy so `content.ts`/`CONTENT.md` untouched; new `past-work-card-links.test.mjs` scans built HTML for `<a>`-in-`<a>` across every prerendered page (13–14 anchors, vacuity-guarded) and was mutation-proved by re-nesting the anchor; focus ring live-measured 7.59:1 dark / 5.69:1 light vs the card surface (AA ≥3:1); holding-state A/B rebuilt on HEAD's component vs the new one with `items: []` → DIFFERING_BYTES 0; 22/22-point `elementFromPoint` hit-test found no dead click zones, real mouse click + real Tab/Enter both navigated; 82 pass/0 fail/0 skip; 602b8ac |
| 5 — Seed DeLuca's mock bridge (demo dataset) | ✅ done [full, flag:data-path] — cross-repo in `~/bcns-client-delucas`, branch `feat/demo-seed-fixture`, final commit 60c1247. `VITE_DEMO_SEED=1 pnpm dev` overlays the mock bridge with a deterministic invented 12-month dataset (all 6 categories) — Dashboard renders $25,111.58 in / $23,875.89 spent / $1,235.69 profit, 12 populated bars. Default `pnpm dev` payload byte-identical to bc60444; seed dead-code-eliminated from every production path (`pnpm verify:no-demo`); 5 data-safety mutations verified to fail the suite; tab-switch render median 21.5ms |
| 6 — L2 Detailz demo seed fixture | ✅ done [full, flag:data-path] — cross-repo in `~/bcns-client-l2detailz`, branch `worktree-demo-seed`, commit 1319991. Opt-in `supabase/demo-seed.sql`: 16 invented identities, 16 bookings, 12 jobs on 9 distinct HST days of the CURRENT month (computed at apply time, so item 7 needs no calendar navigation and the fixture never goes stale). Applied only via `DEMO_SEED=1 bash supabase/local-test/run.sh`; an in-file psql guard makes a bare `psql -f` refuse and write 0 rows. QA PASS (rendered calendar 9 days, 19.7ms median); final review 0 Critical / 0 Important |
| 7 — Capture the three case study screenshots | ✅ done [light] — three PNGs in `apps/web/public/case-studies/` (delucas-dashboard 187KB, l2detailz-frontend 331KB, l2detailz-calendar 300KB), all captured via Claude-in-Chrome from locally-hosted apps on their demo fixtures; registry `screenshots[].src` + `alt` wired and CONTENT.md mirrored; `caption` deliberately left `[INPUT: …]` (public prose about a named client — item 9 owns it behind the permission gate); L2 hero re-aimed at the homepage pricing section because photographic content could not fit under 400KB as lossless PNG; orchestrator personally read `l2detailz-calendar.png` and confirmed all 12 visible jobs use invented demo names with no addresses/phones/emails; 82→87 pass/0 fail/0 skip; 646c6ab. **⚠️ Caught mid-item: the first L2 dev server on :3100 was reading the client's PRODUCTION database — see the security note in the log below. No capture was taken from it.** |
| 8 — Render screenshots with `next/image` | ✅ done [light+ui] — each `/work/[slug]` renders its `screenshots[]` as `<Reveal as="figure">` → `Card` mat (`p-2 sm:p-3`) → `next/image` → `<figcaption>`, staggered `delay={index * 120}`; captions still `[INPUT: …]` by design (item 9 owns them). New `lib/case-study-images.ts` holds a map of **static** PNG imports keyed by registry `src` — `content.ts` can't hold them (the suite loads it under `--experimental-strip-types`, which can't import a `.png`) — which is what makes a bad `src` a BUILD failure and supplies intrinsic width/height with zero hardcoded dimensions. `sharp@0.35.3` added to clear Next's production-optimizer warning. Both failure shapes mutation-proved: deleted PNG → `Module not found`, unregistered `src` → `CaseStudyImageNotFoundError`, both exit 1, both restored via `cp` + `shasum -c`. QA added a reverse-drift test (orphan map entry builds clean otherwise). Live prod smoke: 200/200, every `alt` exact-match, every caption verbatim and after its image, 24 `/_next/image` URLs all 200 with real bytes; `loading="lazy"` ×2 on l2detailz / ×1 on delucas, zero `eager`/`fetchpriority`; zero `next/image` build warnings; median render **2.4ms**; no horizontal overflow in either theme. **Mobile proved structurally, not visually** — `resize_window` no-ops in this sandbox. 87→89 pass/0 fail/0 skip; 26cc92d |
...
- **2026-07-28** — ✅ **Past-work case study system complete to the STOP marker** (`/dev-team-auto`, branch `worktree-past-work-case-studies`, draft PR #14). This session ran items 7 and 8; items 1–6 landed in the prior session. `/work` now works end to end: cards link through to `/work/[slug]` detail pages that render real product screenshots. Suite **89 pass / 0 fail / 0 skip**, build clean at 15 static pages with zero `next/image` warnings, median `/work/l2detailz` render 2.4ms.

  **Item 7 — screenshots captured.** Three PNGs in `apps/web/public/case-studies/`, all via Claude-in-Chrome against locally-hosted apps on their demo fixtures: DeLuca's Dashboard (Vite browser mode on :3001, `VITE_DEMO_SEED=1`), L2's public homepage and admin calendar (:3100 against the seeded local `l2detailz_test` DB). All ≥1200px wide and under 400KB. The L2 hero shot was re-aimed at the homepage's pricing section because photographic content would not fit under 400KB as lossless PNG. Captions were deliberately left as `[INPUT: …]`; `alt` text is real and descriptive.

  **⚠️ Data-safety near-miss — the reason the plan's global rules exist.** The first L2 dev server started for this item was reading the client's **production database**. `.env.local` in the demo-seed worktree carries the live project URL, anon key, service-role key, and a production pooler `DATABASE_URL`, and `lib/db/index.ts` picks the Supabase adapter on the presence of `NEXT_PUBLIC_SUPABASE_URL` — checked *before* `DATABASE_URL`. Two decoys made it look local: pg-boss logged a connection timeout (the pooler's :6543 is egress-blocked here) and the app printed `email worker disabled: no DB` — but Supabase REST over :443 answered 401, i.e. up, and that is the transport the adapter actually uses. Caught before any capture; all three PNGs were taken from a server restarted with the four Supabase vars forced empty and a local `DATABASE_URL`. `.env.local` was **not** edited — see the Needs-Nate item under Current position.

  **Item 8 — screenshots rendered.** Each `/work/[slug]` renders its `screenshots[]` as `<Reveal as="figure">` → `Card` mat → `next/image` → `<figcaption>`, staggered `delay={index * 120}`. New `apps/web/lib/case-study-images.ts` holds a map of **static** PNG imports keyed by registry `src` — `content.ts` cannot hold them because the suite loads it under `--experimental-strip-types`, which cannot import a `.png`. That map is what turns a bad `src` into a build failure and supplies intrinsic dimensions with nothing hardcoded. Both failure shapes were mutation-proved (deleted PNG → `Module not found`; unregistered `src` → `CaseStudyImageNotFoundError`; both exit 1, both restored via `cp` + `shasum -c`), and QA added a reverse-drift test for orphan map entries. **Adding a screenshot now means editing both `content.ts` and `lib/case-study-images.ts`** — a real sync point, gated in both directions.

  **Two things to know before publishing.** `sharp@0.35.3` was added as a direct dependency of `apps/web` to clear Next's production-optimizer warning (Next 14 dropped the squoosh fallback); "zero `next/image` warnings" is only true with it installed. And mobile layout was proved **structurally, not visually** — `resize_window` silently no-ops in this sandbox, so no one looked at a real 390px viewport; the structural proof is no fixed widths in the wrapper chain, `h-auto w-full` inside `max-w-2xl` inside `Container`, an already-`overflow-hidden` section root, and `scrollWidth === clientWidth` in both themes at the achievable width.
