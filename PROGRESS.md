...
| A2 — Scaffold `templates/hosted-web/` starter wired to `@bcns/app-core` | done [full] — @bcns/hosted-web-template (Next.js App Router, templates/* added to workspace glob); env-driven config, opt-in AI (fail-closed), Stripe webhook routes through app-core + now fail-closed (501 when secret set); Dockerfile+DEPLOY.md; no secrets; QA PASS live-smoke + security review 1 Important fixed; commit 3a11c62 |
| A3 — Architecture decision record for the hosted-web model | done [light] — docs/architecture/hosted-web-model.md ADR: pricing+BYOK, hosting stack + cost + 4 risks, per-client-repo (reverses Part II), package/template propagation; QA PASS 18/18; commit 3cac7b5 |
| A4 — Update repo docs to the per-client-repo model | done [light] — CLAUDE.md + README.md reframed to per-client repos from templates/hosted-web/, both cite the ADR, monorepo = platform repo; QA caught+fixed inaccurate app-core description; commit 49d72eb |
